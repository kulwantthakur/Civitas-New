    ...
    public function handle()
    {
        ...
        foreach ($bulletinFolders as $folderPath) {
            ...
            try {
                ...
                // Number is now taken from 'title' field, no folder name extraction

                $existingPage = Page::where('number', $bulletinData['title'])
                    ->where('year', $bulletinData['year'] ?? null)
                    ->where('period', $bulletinData['period'] ?? null)
                    ->first();

                if ($existingPage) {
                    if ($duplicateAction === 'skip') {
                        $this->warn("Skipping duplicate page for folder: {$folderName} (Number: {$bulletinData['title']}, Year: {$bulletinData['year']}, Period: {$bulletinData['period']})");
                        continue;
                    } elseif ($duplicateAction === 'update' && !$dryRun) {
                        $this->info("Updating existing page for folder: {$folderName}");
                        $existingPage->user_id = $userId;
                        $existingPage->section_id = $sectionId;
                        $existingPage->category = $category;
                        $existingPage->url = $this->generateUrl($bulletinData['titre'] ?? $folderName);
                        $existingPage->number = $bulletinData['title'] ?? null;
                        $existingPage->period = $bulletinData['period'] ?? null;
                        $existingPage->year = $bulletinData['year'] ?? null;
                        $existingPage->title = $bulletinData['titre'] ?? 'Untitled';
                        $existingPage->subtitle = $bulletinData['author'] ?? null;
                        $existingPage->content = $bulletinData['description'] ?? null;
                        $existingPage->content_sec = $bulletinData['extra'] ?? null;
                        $existingPage->link = null;
                        $existingPage->upload_video = null;
                        $existingPage->pdf = null;
                        $existingPage->icon = null;
                        $existingPage->image = null;
                        $existingPage->image_responsive = null;
                        $existingPage->events_image = null;
                        $existingPage->html_soruce = null;
                        $existingPage->updated_at = Carbon::now();

                        $existingPage->save();
                        $this->processFiles($folderPath, $existingPage);
                        $importedCount++;
                        $this->line("✓ Updated: {$existingPage->title}");
                        $processedFolders++;
                        continue;
                    }
                }
                ...
                if ($dryRun) {
                    ...
                    $newPage->url = $this->generateUrl($bulletinData['titre'] ?? $folderName);
                    $newPage->number = $bulletinData['title'] ?? null;
                    $newPage->period = $bulletinData['period'] ?? null;
                    $newPage->year = $bulletinData['year'] ?? null;
                    $newPage->title = $bulletinData['titre'] ?? 'Untitled';
        ...
        $this->info("\n=== Import Summary ===");
        $this->info("Successfully imported/updated: {$importedCount} pages");
        if ($errorCount > 0) {
